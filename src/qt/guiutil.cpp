...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("onedollar"))
        return false;
...
{
    // Convert onedollar:// to onedollar:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("onedollar://"))
    {
        uri.replace(0, 12, "onedollar:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "OneDollar.lnk";
}
...
{
    return GetAutostartDir() / "onedollar.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("OneDollar-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  onedollar-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("OneDollar-Qt"));
